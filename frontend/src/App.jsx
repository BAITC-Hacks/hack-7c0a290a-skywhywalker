...
import AgentInvestigator from './components/AgentInvestigator';
...
export default function App() {
  ...
  const [agentResult,setAgentResult]=useState(null),[agentRunning,setAgentRunning]=useState(false),[agentTarget,setAgentTarget]=useState('');
  ...
  function select(id,list=ranking) {
    ...
    setExplanation(null);setMessages([]);setAgentResult(null);setTab('details');setError('');setPriority('ALL');setPattern('ALL');setRole('ALL');
  }
  function clearDataset() {
    setGraph(null);setRanking([]);setNode(null);setSelected('');currentNode.current='';setSearch('');setExplanation(null);setMessages([]);setAgentResult(null);setClusters([]);setComparison(null);setBriefing(null);setEvidence(null);setModal(null);resetView();
  }
  ...
  function investigate(question=''){
    if(!selected||agentRunning||busy)return;
    const id=selected,version=epoch.current;setTab('agent');setAgentResult(null);setAgentTarget(id);setAgentRunning(true);
    run(async()=>{try{const result=await api('/ai/investigate',{node_id:id,question});if(version===epoch.current&&id===currentNode.current)setAgentResult(result);}finally{setAgentRunning(false);}});
  }
  function focusWorkspace(){setModal(null);document.getElementById('graph-heading')?.scrollIntoView({behavior:'smooth',block:'start'});}
  function openAgent(){setTab('agent');document.getElementById('detail-heading')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
  ...
  return <div className="app-shell">
    <Sidebar ranking={ranking} selected={selected} onSelect={select} onSummary={summary} onClusters={showClusters} onHelp={()=>setModal('help')} onAgent={openAgent} onJury={()=>setModal('jury')} busy={busy} filename={filename} sourceName={sourceName}/>
    <main className="main">
      <header className="topbar"><div><div className="breadcrumb"><span className="live-dot"/> AML WORKSPACE <ChevronRight size={12}/> Обзор расследования</div><h1>Кого проверить первым?</h1><p className="page-subtitle">Найдите ключевые счета, проследите деньги и проверьте основания.</p></div><button className="jury-entry" onClick={()=>setModal('jury')}><FileSearch size={17}/>Обзор за 60 секунд<ChevronRight size={15}/></button></header>
      <div className="workflow-strip" aria-label="Сценарий проверки"><button onClick={focusWorkspace}><span>1</span><b>Выберите счёт</b><small>Очередь слева</small></button><ChevronRight size={16}/><button disabled={!node} onClick={()=>showEvidence()}><span>2</span><b>Проверьте основания</b><small>Связи и переводы</small></button><ChevronRight size={16}/><button disabled={!node} onClick={openAgent}><span>3</span><b>Запустите AI-проверку</b><small>Инструменты и факты</small></button></div>
      <StatsBar stats={graph?.stats} ranking={ranking}/>
      <div className="data-actions"><label className="data-picker"><Database size={15}/><select aria-label="Выбрать набор данных" value={datasetId} onChange={event=>changeDataset(event.target.value)} disabled={busy}>{datasets.map(item=><option key={item.id} value={item.id}>{item.id==='official'?'Данные хакатона':item.id==='demo'?'Учебный пример':item.name}</option>)}{!datasets.some(item=>item.id===datasetId)&&<option value={datasetId}>{sourceName}</option>}</select></label><UploadPanel busy={busy} onUpload={upload} onAnalyze={analyze} onExport={exportResults} hasGraph={Boolean(graph)}/></div>
      ...
      <div className="workspace">
        <section className="graph-panel">
          <div className="graph-header"><div><h2 id="graph-heading"><Network size={18}/>Куда движутся деньги</h2><p>{graph?`${dateLabel(graph.stats.start,'day')} — ${dateLabel(graph.stats.end,'day')}${graph.stats.time_precision==='day'?'':' · UTC'}`:'Загрузите CSV, Parquet или ZIP'}</p></div><form className="search graph-header-search" onSubmit={find}><Search size={15}/><input aria-label="Полный идентификатор счёта" placeholder="Найти счёт по GID" value={search} onChange={event=>setSearch(event.target.value)}/><button aria-label="Найти счёт во всём наборе" type="submit">↵</button></form></div>
          <details className="depth-settings"><summary>Глубина обхода и сброс фильтров</summary><div className="graph-toolbar"><div className="depth-control"><span title="1 — прямые соседи; 4 — связи на расстоянии до четырёх переводов.">Переходов</span>{[1,2,3,4].map(value=><button key={value} className={depth===value?'active':''} disabled={!selected} onClick={()=>{setDepth(value);setFollowing(true);}} aria-pressed={depth===value}>{value}</button>)}</div><button className="text-button" onClick={resetView}>Сбросить фильтры</button></div></details>
          <details className="advanced-filters"><summary><SlidersHorizontal size={14}/>Фильтры и цвет графа{(priority!=='ALL'||role!=='ALL'||pattern!=='ALL'||minAmount||maxAmount)&&<span>Есть активные</span>}</summary><div className="filter-bar"><select aria-label="Фильтр по приоритету" value={priority} onChange={event=>setPriority(event.target.value)}>{Object.entries(levels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select aria-label="Фильтр по роли" value={role} onChange={event=>setRole(event.target.value)}><option value="ALL">Все роли</option>{Object.entries(roles).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select aria-label="Фильтр по особенностям" value={pattern} onChange={event=>setPattern(event.target.value)}><option value="ALL">Все особенности</option>{patterns.map(value=><option key={value} value={value}>{patternLabel(value)}</option>)}</select><input type="number" min="0" aria-label="Минимальная сумма отдельного перевода" placeholder="Сумма от" value={minAmount} onChange={event=>setMinAmount(event.target.value)}/><span>–</span><input type="number" min="0" aria-label="Максимальная сумма отдельного перевода" placeholder="Сумма до" value={maxAmount} onChange={event=>setMaxAmount(event.target.value)}/><select aria-label="Цвет счетов на графе" value={colorMode} onChange={event=>setColorMode(event.target.value)}><option value="roles">Цвет: роли</option><option value="priority">Цвет: приоритет</option><option value="communities">Цвет: группы</option></select></div></details>
        ...
        <aside className="detail-panel"><div className="panel-tabs" id="detail-heading"><button className={tab==='details'?'active':''} onClick={()=>setTab('details')}><FileSearch size={15}/>О счёте</button><button className={tab==='agent'?'active':''} onClick={()=>setTab('agent')}><Sparkles size={15}/>AI-проверка</button><button className={tab==='copilot'?'active':''} onClick={()=>setTab('copilot')}>Вопрос</button></div><div className="detail-content">{tab==='details'?<NodeDetails node={node} metadata={graph?.stats} onFollow={()=>following?resetView():setFollowing(true)} onAI={openAgent} onEvidence={()=>showEvidence()} onCompare={openCompare} following={following} busy={busy}/>:tab==='agent'?<AgentInvestigator key={selected} node={node} result={agentResult} running={agentRunning&&agentTarget===selected} busy={busy} onRun={investigate} onEvidence={showEvidence} onSelect={selectFromModal}/>:<AICopilot node={node} result={explanation} messages={messages} onExplain={askExplain} onAsk={ask} onEvidence={showEvidence} busy={busy}/>}</div></aside>
      </div>
    ...
    {modal==='jury'&&<Modal title="Проверка проекта за 60 секунд" onClose={()=>setModal(null)}><JuryGuide onStart={focusWorkspace}/></Modal>}
