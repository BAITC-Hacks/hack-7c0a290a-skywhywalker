...
export default function Sidebar({ranking,selected,onSelect,onSummary,onClusters,onHelp,onAgent,onJury,busy,filename,sourceName}) {
  return <aside className="sidebar">
    ...
    <nav>
      ...
      <button className="nav-item" onClick={onAgent} disabled={!ranking.length}><Sparkles size={18}/>AI-проверка<ArrowUpRight size={14}/></button>
      <button className="nav-item" onClick={onHelp}><CircleHelp size={18}/>Как это работает</button>
      <button className="nav-item jury-nav" onClick={onJury}><ShieldCheck size={18}/>О проекте и критериях</button>
    </nav>
