...
export default function UploadPanel({busy,onUpload,onAnalyze,onExport,hasGraph}) {
  ...
  return <div className="upload-actions">
    ...
    <button className={hasGraph?'':'primary'} disabled={busy} onClick={onAnalyze}>{busy?<LoaderCircle size={17} className="spin"/>:<Play size={17}/>} {hasGraph?'Пересчитать':'Построить граф'}</button>
