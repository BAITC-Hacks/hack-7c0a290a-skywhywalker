...
export default function MoneyGraph({graph,selected,onSelect,priority,pattern,minAmount,maxAmount,followIds,resetKey}) {
 ...
 useEffect(()=>{
  ...
  const instance=cytoscape({container:container.current,elements:[...nodes,...graph.edges],minZoom:.12,maxZoom:4,wheelSensitivity:.18,layout:{name:'cose',randomize:false,animate:false,nodeRepulsion:()=>16000,idealEdgeLength:()=>100,gravity:.2,numIter:700,padding:55},style:[
   {selector:'node',style:{'background-color':'#3b826b','border-color':'#7fd4b0','border-width':1.5,'label':'data(label)','font-family':'Inter, Segoe UI, sans-serif','font-size':12,'font-weight':600,'color':'#bbcec8','text-valign':'bottom','text-margin-y':8,'text-outline-color':'#101820','text-outline-width':2,'width':'mapData(priority_score,0,100,18,48)','height':'mapData(priority_score,0,100,18,48)'}},
   {selector:'node[priority_level="MEDIUM"]',style:{'background-color':'#795b2f','border-color':'#e8b067','color':'#e2c49b'}},
   {selector:'node[priority_level="HIGH"]',style:{'background-color':'#843f4a','border-color':'#ef8291','color':'#f2bac3'}},
   {selector:'edge',style:{'width':1,'line-color':'#33454f','target-arrow-color':'#50616b','target-arrow-shape':'triangle','arrow-scale':.6,'curve-style':'bezier','opacity':.55}},
   {selector:'.focused',style:{'border-width':4,'border-color':'#d9fff0','overlay-color':'#71e8b6','overlay-opacity':.12,'overlay-padding':9,'z-index':20}},
   {selector:'.flow',style:{'line-color':'#5dd3a5','target-arrow-color':'#5dd3a5','width':2,'opacity':.9}},
 ...
 return <div className="graph-canvas-wrap"><div className="graph-canvas" ref={container} role="img" aria-label="Интерактивный граф транзакций. Выберите account из доступного списка слева."/><div className="graph-caption"><span className="live-dot"/> DIRECTED TRANSACTION GRAPH <small>{graph?.stats.links??0} уникальных связей</small></div><div className="graph-controls"><button title="Приблизить" aria-label="Приблизить" onClick={()=>cy.current?.zoom(cy.current.zoom()*1.2)}><Plus size={18}/></button><button title="Отдалить" aria-label="Отдалить" onClick={()=>cy.current?.zoom(cy.current.zoom()/1.2)}><Minus size={18}/></button><button title="Вместить граф" aria-label="Вместить граф" onClick={()=>cy.current?.fit(undefined,55)}><Maximize size={17}/></button><button title="Сбросить расположение" aria-label="Сбросить расположение" onClick={()=>cy.current?.layout({name:'cose',randomize:false,animate:false,padding:55}).run()}><RotateCcw size={17}/></button></div><div className="graph-legend"><span><i className="dot low-bg"/>Low</span><span><i className="dot medium-bg"/>Medium</span><span><i className="dot high-bg"/>High priority</span><span className="legend-hint">Размер = приоритет · Стрелка = направление</span></div></div>
}
