...
export async function api(path, body, options={}) {
  ...
  let data; try {data=await res.json();} catch {throw new Error('Сервер вернул некорректный ответ. Проверьте запуск backend.');}
...
export const short = v => new Intl.NumberFormat('en',{notation:'compact',maximumFractionDigits:1}).format(v??0);
export const patternLabel = p => p==='POTENTIAL_MULE_PATTERN'?'Potential mule-related pattern':p.replaceAll('_',' ');
