...
Интерфейс, подсказки, основания оценки и резервные объяснения переведены на русский. Раздел «Как это работает» объясняет круги, стрелки, приоритет и роль ИИ на простом примере. Светлая тема с зелёными акцентами вдохновлена официальным сайтом Freedom Bank: https://www.bankffin.kz/ru . MoneyGraph — самостоятельный прототип, а не официальный сервис банка.
