...
def client(monkeypatch):
    ...
    with TestClient(app) as c:
        c.headers['X-Dataset-ID']='demo'
        c.get('/api/graph')
        yield c
    for k in list(store):
        if k not in {'demo','official'}:del store[k]
...
def test_small_and_self_loop_datasets():
    ...
    assert a.nodes['A']['features']['rapid_share'] is None
    assert a.nodes['A']['features']['timing_available'] is False
