def detect_patterns(features, graph):
    ...
    for n, f in features.items():
        ...
        if f.get('timing_available',True) and not f.get('is_seed') and not f.get('truncated_by_depth') and (f['rapid_share'] or 0) >= .6 and f['turnover_ratio'] >= .7:
        ...
        mule = {'multiple_sources':f['incoming_counterparties'] >= 5,
                'rapid_forwarding':bool(f.get('timing_available',True) and not f.get('is_seed') and not f.get('truncated_by_depth') and (f['rapid_share'] or 0) >= .6),
